...
def CreatePath(case_number):
    path_ref = 'G:/Documents/wasa_tests/OutputsREF/Output%d'% (case_number)
    path_new = 'G:/Documents/wasa_tests/Outputs/Output%d'% (case_number)
...
### For the moment we remove also the files river_storage and susp_sediment_storage because I did not find a way to read them => adapt the code
...
##### Dataframe creation from a file name.
###It takes a file name and the path to have access to it and then convert the file into a dataframe
## First reading mode
def CreateDf1(file_name,path): #path = where is the file
    os.chdir(path)
    ...
    return df
## Second reading mode
def CreateDf2(file_name,path): #path = where is the file
    os.chdir(path)
...
### 2) If not, with dataframe.compare() we get an df with the data which are different between the two df. we check if the resulting df has float or string data
### => if there are float data (the majority of the files), we proceed as follow:
...
### => if there are no float data so string data, we read the list differently:
###         -> First we have to through the list to convert the string into float data
###         Note: the organization is not the same as in the previous case with directly float data 
###         -> Then we compare the data and calculate the error as in the previous case
...
def GeneralComparison(file_name, dfRef, dfNew, error_acceptable, textFile):
    ...
    if TF == True:
        ...
        S = []
        if isinstance(CompList[0][0], float)==True: #we check if into the list we directly have float data (in the most cases) or string data 
            for j in range(n):
            ...
            CompList_Ref=[] #it will take the values corresponding to the reference file
            CompList_New=[]
            for j in range(n): #we go through the list CompList which contains lists of strings such as: '  10000  2   2  1  '
                CL_Ref_line=[]
                CL_New_line=[]
...
##### We take two data frames and check if there is an error message such as 'ValueError' when we use the function df.compare()
...
##### This function reads the files with the adapted reading mode
def ReadDf(file_name, pathA, pathB):
    dfA = CreateDf1(file_name, pathA) # we read with the classic mode
    dfB = CreateDf1(file_name, pathB)
    if ComparisonCheck(dfA, dfB)==True: # if there is no problem when comparing them we keep the df we got
        return dfA, dfB
    else: #if there is an issue, we read the file with the second mode and so we will get string data into the CompList in the GeneralComparison function
...
def MainLoop(pathRef, pathNew, namesList, errorAcceptable, textFile):
    ...
    for name in namesList: #take each name in the file names list
        df_ref,  df_new = ReadDf(name, pathRef, pathNew) #create the dataframe for each version from the file name
